...
@Controller
@Slf4j
public class BankingDataRecipientBrandSoftwareProductApiDelegateImpl implements BankingDataRecipientBrandSoftwareProductApiDelegate {
  ...
  @Override
  public ResponseEntity<DioSoftwareProduct> createRecipientBrandSoftwareProduct(UUID recipientId, UUID brandId,
      DioSoftwareProduct softwareProduct) throws ValidationListException {

    Optional<DataRecipientBrandData> brand = brandRepository.findByIdAndDataRecipientId(brandId, recipientId);

    if (!brand.isPresent()) {
      LOG.warn("Attempted to create a software product for a recipient id of {} and brand id of {} that doesn't exist",
          recipientId, brandId);
  ...
  public ResponseEntity<List<DioSoftwareProduct>> listRecipientBrandSoftwareProducts(UUID recipientId, UUID brandId) {
    ...
    return ResponseEntity.ok(mapper.mapAsList(
        Optional.of(recipient.get().softwareProducts()).orElse(Set.of()), DioSoftwareProduct.class));
  }

  @Override
  public ResponseEntity<DioSoftwareProduct> getRecipientBrandSoftwareProduct(UUID recipientId, UUID brandId, UUID softwareProductId) {
    Optional<SoftwareProductData> data = softwareProductRepository.findByIdAndDataRecipientBrandIdAndDataRecipientBrandDataRecipientId(softwareProductId, brandId, recipientId);

    if (data.isPresent()) {
      LOG.info("Retrieving a single data recipient softwareProduct with recipient of {} and softwareProduct of {} and got content of {}", recipientId, softwareProductId,
          data.get());
      return ResponseEntity.ok(mapper.map(data.get(), DioSoftwareProduct.class));
    } else {
      LOG.warn("Attempted to retrieve a single recipient {} with softwareProduct of {} and couldn't find it", 
          recipientId, softwareProductId);
      return new ResponseEntity<>(HttpStatus.NOT_FOUND);
    }  }

  @Override
  public ResponseEntity<DioSoftwareProduct> updateRecipientBrandSoftwareProduct(UUID recipientId, UUID brandId, UUID softwareProductId,
      DioSoftwareProduct updateData) {
    Optional<SoftwareProductData> optionalData = softwareProductRepository.findByIdAndDataRecipientBrandIdAndDataRecipientBrandDataRecipientId(softwareProductId, brandId, recipientId);
    ...
    if (optionalData.isPresent()) {
      ...
      LOG.info("Updating a single data recipient software product of recipient {}, brand {} and software product {} and now set to {}", recipientId, brandId, softwareProductId, updatedData);

      return ResponseEntity.ok(mapper.map(updatedData, DioSoftwareProduct.class));
    } else {
      LOG.warn("Attempted to retrieve a single data recipient softwareProduct and could not find with recipient of {} and softwareProduct of {}",
          recipientId, softwareProductId);
      return new ResponseEntity<>(HttpStatus.NOT_FOUND);
    }
  }
  ...
  @Override
  public ResponseEntity<Void> deleteRecipientBrandSoftwareProduct(UUID recipientId, UUID brandId, UUID softwareProductId) {
    Optional<SoftwareProductData> data = softwareProductRepository.findByIdAndDataRecipientBrandIdAndDataRecipientBrandDataRecipientId(softwareProductId, brandId, recipientId);
    ...
    if (data.isPresent()) {
      ...
      LOG.warn("Attempted to retrieve a single data recipient software product and could not find with recipient of {}, brand of {} and software product of {}",
